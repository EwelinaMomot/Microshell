...
void autocomplete(char *buffer, int *len) {
    ...
    if (*len == 0) {
    return; //jeśli kliknięto TAB bez polecenia, nic nie rób
}
    ...
    if (matches == 1) {
        ...
        *len += to_add;
        ...
        printf("\n> %s", buffer);//wypisujemy ponownie to co uzytkownik wpisal zeby mogl kontynuowac
        fflush(stdout);
    }
        ...
        if (c == 127) {
            if (len > 0) {
                ...
                write(STDOUT_FILENO, "\b \b", 3);
                 /*"\b \b" - specjalna sekwencja dla terminala:
                \b → cofnięcie kursora o 1 znak
                → nadpisanie znaku spacją
                \b → cofnięcie kursora z powrotem

                Efekt wizualny: usuwa ostatni znak z ekranu.*/
...
//Obsługuje sygnał SIGINT (Ctrl+C) tak, aby microshell się nie zamykał, tylko przechodził do nowej linii.
...
int main() {
    ...
    //ustawia microshell jako lidera własnej grupy procesów, co jest konieczne do poprawnego zarządzania terminalem i sygnałami job control.
    shell_pgid = getpid();
    ...
    while (1) {
        ...
        if (strcmp(argv[0], "cd") == 0) {
            ...
            while (arg_start < argc && argv[arg_start][0] == '-') {
            if (strcmp(argv[arg_start], "-i") == 0) {
                interactive = 1;
            } else if (strcmp(argv[arg_start], "-b") == 0) {
                backup = 1;
            } else {
                printf("mv: unknown option %s\n", argv[arg_start]);
                break;
            }
            arg_start++;
        }
